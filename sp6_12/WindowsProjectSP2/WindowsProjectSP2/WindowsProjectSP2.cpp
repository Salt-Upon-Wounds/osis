...
//по хорошему они должны быть в ресурсах, но кому какая разница
#define EDIT_ID 1
...
//дескрипторы для окна, кнопок, списков
...
//чтобы не вставлять одинаковые строки, сравниваем их
bool ContainsString(HWND hListBox, TCHAR* s) {
	...
	for (int i = 0; i < count; i++) {
		...
		if (!_tcscmp(s, buffer))//типо strcmp
			return true;
	}
...
//удаляем выделенные пункты
bool DeleteSelectedItem(HWND hListBox) {
	int selCount = SendMessage(hListBox, LB_GETSELCOUNT, 0, 0);//количество выделенных
	int countBuffer[100];
	SendMessage(hListBox, LB_GETSELITEMS, 100, (LPARAM)countBuffer);//осбственно выделенные

	for (int i = selCount - 1; i >= 0; --i) {
		int itemId = countBuffer[i];
		SendMessage(hListBox, LB_DELETESTRING, itemId, 0);//удаляем
	}
